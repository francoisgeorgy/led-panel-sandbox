...
class SampleBase(object):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.parser.add_argument("-r", "--led-rows", action="store", help=f"Display rows. 16 for 16x32, 32 for 32x32. Default: 64", default=64, type=int)
        self.parser.add_argument("-l", "--led-cols", action="store", help=f"Panel columns. Typically 32 or 64. (Default: 128)", default=128, type=int)
        ...
        if running_on_pi:
            ...
            self.parser.add_argument("--led-slowdown-gpio", action="store", help=f"Slow down writing to GPIO. Range: 0..5. Default: 4", default=4, type=int)
            self.parser.add_argument("--led-no-hardware-pulse", action="store", help="Don't use hardware pin-pulse generation")

            self.parser.add_argument("--led-rgb-sequence", action="store", help="Switch if your matrix has led colors swapped. Default: RGB", default="RGB", type=str)

            self.parser.add_argument("--led-pixel-mapper", action="store", help=f"Apply pixel mappers. Default: \"\"", default="", type=str)
    ...
    def rectangle(self, x0, y0, x1, y1, color, fill=False):
        if fill:
            for y in range(y0, y1 + 1):
                rgb_graphics.DrawLine(self.canvas, x0, y, x1, y, color)
        else:
            rgb_graphics.DrawLine(self.canvas, x0, y0, x1, y1, color)
            rgb_graphics.DrawLine(self.canvas, x1, y0, x1, y1, color)
            rgb_graphics.DrawLine(self.canvas, x0, y1, x0, y1, color)
            rgb_graphics.DrawLine(self.canvas, x0, y0, x0, y1, color)

    def circle(self, x0, y0, r, color):
        rgb_graphics.DrawCircle(self.canvas, x0, y0, r, color)

    def text(self, x, y, color, text, font):
        rgb_graphics.DrawText(self.canvas, font, x, y, color, text)
